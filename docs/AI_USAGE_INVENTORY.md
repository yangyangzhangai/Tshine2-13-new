...
| 待办拆解 | `api/todo-decompose.ts` + `src/server/todo-decompose-service.ts`；入口：`src/features/growth/SubTodoList.tsx` | zh 默认 DeepSeek `deepseek-chat`（`TODO_DECOMPOSE_MODEL_ZH`），en/it 默认 Gemini `gemini-2.5-flash`（`TODO_DECOMPOSE_MODEL`，404 下线自动降级 `TODO_DECOMPOSE_GEMINI_FALLBACK_MODEL`） | 将待办拆成 3-5 个可执行步骤并给时长 | 指令遵循、JSON 稳定、轻规划能力 | 7B-20B |
...
- `/api/todo-decompose` -> `DEEPSEEK_API_KEY`（zh，`TODO_DECOMPOSE_MODEL_ZH`）+ `GEMINI_API_KEY`（en/it，`TODO_DECOMPOSE_MODEL`）；可选 `DEEPSEEK_BASE_URL`、`TODO_DECOMPOSE_GEMINI_BASE_URL`、`TODO_DECOMPOSE_GEMINI_FALLBACK_MODEL`、`TODO_DECOMPOSE_VERBOSE_LOGS`
