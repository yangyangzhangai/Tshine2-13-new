  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 429) {
        throw new Error('API 调用次数超限 (429)，请稍后再试或检查配额。');
      }
...
* **红黑榜**：
    ...
    * ⚠️ **崩溃/低效时刻**：本周最糟糕的时间段（如“周三下午集体性摸鱼”），并简要分析触发机制（是情绪崩溃、外部打断还是生理疲劳）。
...
export const generateReportAnalysis = async (data: any, type: 'daily' | 'weekly' | 'monthly' | 'custom') => {
...
`;
  ...
  return chatWithQwen([{ role: 'user', content: context }]);
};
