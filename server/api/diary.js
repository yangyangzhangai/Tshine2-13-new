/**
 * Express API Handler - Shadow Diary (观察手记) API
...
const getDiarySystemPrompt = (userName) => {
...
const getDiarySystemPromptEn = (userName) => {
...
export default async function handler(req, res) {
  ...
  if (userName) {
    if (lang === 'en') {
      finalSystemPrompt += `\n\n【IMPORTANT CRITICAL RULE】: The user's name is "${userName}". You MUST refer to the user by this name ("${userName}") instead of using generic terms like "them", "the user", or "my host".`;
    } else {
      finalSystemPrompt += `\n\n【最重要指令】：用户的昵称是"${userName}"。你在日记正文中，绝对禁止使用"ta"或"用户"来称呼对方，必须全程使用"${userName}"来称呼！`;
    }
  }
