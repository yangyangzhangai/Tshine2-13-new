/**
 * Express API Handler - Report Analysis API
...
日期: ${data.date}
待办事项:
${data.todos?.map((t) => `- [${t.completed ? 'x' : ' '}] ${t.content} (优先级: ${t.priority}, 分类: ${t.category})`).join('\n') || '无'}

活动记录:
${data.activities?.map((a) => `- ${a.time}: ${a.content} (耗时: ${a.duration}分钟)`).join('\n') || '无'}
